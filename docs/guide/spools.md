...
Spoolman and printer integrations use explicit external IDs and stock ownership; native stock never assumes that another system has already deducted material. Keep integrations disabled until their ownership mode is configured under **Settings → Features**.
