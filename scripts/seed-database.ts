...
async function assertEmpty(transaction: Transaction) {
...
async function createPrint(transaction: Transaction, spec: DemoPrint) {
  ...
  const calculation = calculatePrintCost({
    ...
    filaments: [
      {
        id: filament.spoolId,
        name: `${filament.name} · ${filament.spoolCode}`,
  ...
  const filamentLine = line(calculation, filament.spoolId);
...
async function seed(credentials: ReturnType<typeof parseCredentials>) {
  await db.$transaction(
    async (transaction) => {
      ...
      for (const print of demoPrints) await createPrint(transaction, print);
    },
