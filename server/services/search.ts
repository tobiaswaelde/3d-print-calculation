...
export async function searchApplication(query: Record<string, unknown>) {
  ...
  const [prints, customers, printers, components, filaments] = await db.$transaction([
    ...
    db.filament.findMany({
      ...
      select: { id: true, name: true, manufacturer: true, material: true, color: true },
  ...
  const groups: GlobalSearchGroup[] = [
    ...
    },
    {
      ...
      items: components.map((item) => ({
        id: item.id,
        title: item.name,
        description: [item.type, item.manufacturer, item.model].filter(Boolean).join(' · '),
    ...
    },
    {
      ...
      items: filaments.map((item) => ({
        id: item.id,
        title: item.name,
        description: [item.manufacturer, item.material, item.color].filter(Boolean).join(' · '),
